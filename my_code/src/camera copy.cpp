...
void Camera::captureThread() {
    ...
    while (!m_stopFlag) {
        ...
        uint8_t* yuyv = (uint8_t*)m_buffers[buf.index].start;
        x264_picture_t* pic = (x264_picture_t*)m_picture;
        
        // YUYV到I420的转换
        for (int i = 0; i < HEIGHT; i++) {
            for (int j = 0; j < WIDTH; j++) {
                pic->img.plane[0][i * pic->img.i_stride[0] + j] = yuyv[i * WIDTH * 2 + j * 2];
                if (i % 2 == 0 && j % 2 == 0) {
                    pic->img.plane[1][i/2 * pic->img.i_stride[1] + j/2] = yuyv[i * WIDTH * 2 + j * 2 + 1];
                    pic->img.plane[2][i/2 * pic->img.i_stride[2] + j/2] = yuyv[i * WIDTH * 2 + j * 2 + 3];
                }
            }
        }
